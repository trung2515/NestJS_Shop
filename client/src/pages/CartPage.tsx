...
export function CartPage({ onCartChange }: { onCartChange: (count: number) => void }) {
  ...
  return (
    <Stack spacing={3}>
      <Typography variant="h4" fontWeight={900}>Gio hang</Typography>
            ...
            <Paper key={item.id} elevation={0} sx={{ p: 2, border: '1px solid #e5e7eb' }}>
              <Stack direction="row" spacing={2} alignItems="center">
                ...
                <Box flexGrow={1}>
                  ...
                  <Typography color="primary" fontWeight={900}>{Number(item.product.price).toLocaleString('vi-VN')} d</Typography>
                </Box>
                <Button color="error" startIcon={<DeleteIcon />} onClick={() => removeItem(item.id)}>Xoa</Button>
              </Stack>
            </Paper>
          ))}
          {items.length === 0 && <Paper elevation={0} sx={{ p: 3, border: '1px solid #e5e7eb' }}>Gio hang dang trong.</Paper>}
        </Stack>

        <Paper elevation={0} sx={{ p: 3, border: '1px solid #e5e7eb', height: 'fit-content' }}>
          <Stack spacing={2}>
            <Typography variant="h6" fontWeight={900}>Thanh toan</Typography>
            <Divider />
            <TextField label="Dia chi giao hang" value={address} onChange={(event) => setAddress(event.target.value)} multiline minRows={3} />
            <Stack direction="row" justifyContent="space-between">
              <Typography>Tong tien</Typography>
              <Typography fontWeight={900} color="primary">{total.toLocaleString('vi-VN')} d</Typography>
            </Stack>
            <Button disabled={items.length === 0} variant="contained" size="large" startIcon={<PaymentIcon />} onClick={checkout}>
              Dat hang
            </Button>
          </Stack>
        </Paper>
